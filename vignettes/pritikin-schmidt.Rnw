...
However, the app does not conduct any analysis.
...
opts_chunk$set(echo=FALSE, cache=FALSE, highlight=FALSE,
               size="footnotesize", background='#ffffff')
...
\pkg{OpenMx}, a modular package originally designed for
...
Here we introduce a new \pkg{shiny} \citep{shiny:webapp} web application
to generate \pkg{OpenMx} code in \pkg{rmarkdown} format \citep{rmarkdown}.
We believe this code generator substantially lowers the barrier
to entry for novice users of Item Factor Analysis
...
with item parameters $\xi_j$ and latent ability $\theta_i$ is
%
\begin{align}
...
assumed to be conditionally independent given the latent ability $\theta$.
...
\end{align}
%
Optimization consists of finding the $\xi$ that maximizes this function \citep{bock1981}.
...
print(pl)
@ 
\caption{How ordinal data is converted into continuous data conditional on examinee skill.}
...
into continuous data conditional an examinee skill.
...
and browse to the output web server address.
As before, note that the port number (\verb\3726\ printed below) may be different on your computer.
...
\caption{After loading the g341-19.csv data.}
...
a dichotomous data set that is available as an online supplement to this article.
The first 6 lines will appear in the \verb\Unparsed content\ section
(see Figure~\ref{fig:g341-19-part1}).\footnote{We realize that these screenshots
...
\caption{A summary of the g341-19.csv data set when parsed incorrectly as a single column.}
...
Click on the \verb\Item summary\ as an alternate way
...
The \verb\Row frequency column\ is used when you have already
reduced your data to unique rows and row counts.
The example data set \verb\LSAT6\ is in this format.
For our current data set, leave the \verb\Row frequency column\ set to $-$.
...
``incorrect'' for the \verb\New name\, and click the
\verb\Add mapping\ button.
This will create a recoding rule that will show up
in the \verb\Recode Table\ (Figure~\ref{fig:g341-19-part4}).
Similarly, rename the ``1'' outcome to ``correct'' and
again click the \verb\Add mapping\ button.
At this point, you should have 2 rules in the \verb\Recode Table\.
...
\caption{Outcome reorder tool with 1 reordering rule.}
...
We will revisit this question in Section~\ref{section:ifa}.
...
\verb\drm\ is an abbreviation for the 4PL dichotomous response model \citep{loken2010},
which has 4 parameters when there is 1 factor.
...
Estimating the lower bound from data without knowing how
many alternatives were presented to the examinee typically
requires much more data than is required to estimate other parameters.
A Bayesian prior is often used on the lower bound
with the mode of the prior set to the expected guessed-correct probability.
This is easily accomplished by dragging the \verb\Prior mode\ slider
and clicking the \verb\Set\ button.
...
The logit reflects that the parameter is estimated on the logit scale,
...
The \verb\Information matrix method\ is set to \verb\Oakes\ by default.
...
Click the \verb\Download\ button, save the \verb\rmarkdown\ code,
open the file in \pkg{RStudio},
and follow the instructions to render it into a web page.
...
imat <- mxMatrix(name='item', values=startingValues, free=!is.na(startingValues)) ## \label{e1:item}
...
priorParam <- mxMatrix(name='priorParam', nrow=1, ncol=length(priorLabels),
...
# Adjust the path in the next statement to load your data
data <- read.csv(file='~/ifa/mbManual/g341-19.csv',header=FALSE,sep='',quote="",stringsAsFactors=FALSE,check.names=FALSE)
...
spec <- list()
spec[1:6] <- rpf.drm(factors=numFactors)
spec[7:12] <- rpf.grm(factors=numFactors,outcomes=2)
names(spec) <- c("V1", "V2", "V3", "V4", "V5", "V6", "V7", "V8", "V9", "V10", 
  "V11", "V12")
...
we obtain some diagnostics upon knitting the Rmarkdown to HTML.
...
[0] ComputeEM: deriv of gradient for 2
[0] ComputeEM: deriv of gradient for 3
[0] ComputeEM: deriv of gradient for 4
[0] ComputeEM: deriv of gradient for 5
[0] ComputeEM: deriv of gradient for 6
[0] ComputeEM: deriv of gradient for 7
[0] ComputeEM: deriv of gradient for 8
[0] ComputeEM: deriv of gradient for 9
[0] ComputeEM: deriv of gradient for 10
[0] ComputeEM: deriv of gradient for 11
[0] ComputeEM: deriv of gradient for 12
[0] ComputeEM: deriv of gradient for 13
[0] ComputeEM: deriv of gradient for 14
[0] ComputeEM: deriv of gradient for 15
[0] ComputeEM: deriv of gradient for 16
[0] ComputeEM: deriv of gradient for 17
[0] ComputeEM: deriv of gradient for 18
[0] ComputeEM: deriv of gradient for 19
[0] ComputeEM: deriv of gradient for 20
[0] ComputeEM: deriv of gradient for 21
[0] ComputeEM: deriv of gradient for 22
[0] ComputeEM: deriv of gradient for 23
...
but the optimum (line~\ref{e2:fit}) should be exactly the same.
...
between \pkg{OpenMx} and \pkg{rpf}.
\pkg{rpf} is an acronym for response probability function
...
The sign of each $p$-value is determined by ordinal gamma,
a measure of association \citep{agresti1990}.
Positive numbers indicate more correlation than expected.
These are cause for concern and suggest local dependence \citet{chen1997}.
...
The local dependence is most severe between items V1-V6
and V7-V12, however, it is not clear why.
Understanding how to improve the model (or the items)
often requires guesswork and trial-and-error.
...
sfit <- SitemFit(m1Grp)
tbl <- t(sapply(sfit, function(r) c(n=r$n, df=r$df, stat=r$statistic, pval=r$pval)))
print(xtable(tbl, paste0('Sum-score item-wise fit'), 'tab:sitemfit'))
@ 
...
This plot is similar to an item characteristic curve plot (Figure~\ref{fig:iccplot}).
The main advantage of \verb\SitemPlot\ over \verb\iccPlot\
is that \verb\SitemPlot\ is well defined regardless of
...
where there is more than 1 latent factor.
...
Exhibited is the \pkg{OpenMx} provided summary of model fit.
...
\pkg{OpenMx}, \pkg{rpf}, and \pkg{ifaTools} are free and open-source software.
...
\section{Polytomous data}\label{sec:polytomous}
...
a data set from \citet{thissen1988} that is available as an online supplement to this article.
Click the \verb\Row names?\ checkbox in the control panel
to disable row names.
...
\begin{wrapfigure}{r}{0.5\textwidth}
\includegraphics[width=0.48\textwidth]{ss12}
...
\begin{wrapfigure}{r}{0.5\textwidth}
\includegraphics[width=0.48\textwidth]{ss13}
...
numFactors <- length(factors)
spec <- list()
spec[1:2] <- rpf.nrm(factors=numFactors,outcomes=4,T.a='trend', T.c='trend')
...
numFactors <- length(factors)
spec <- list()
spec[1:2] <- rpf.nrm(factors=numFactors,outcomes=4,T.a='trend', T.c='trend') ## \label{e3:nominal}
names(spec) <- c("Match", "Identify")
...
sfit <- SitemFit(m1Grp)
tbl <- t(sapply(sfit, function(r) c(n=r$n, df=r$df, stat=r$statistic, pval=r$pval)))
print(xtable(tbl, paste0('Sum-score item-wise fit'), 'tab:e3-sitemfit'))
@ 
...
The sum-score item fit tests are not statistically significant (Table~\ref{tab:e3-sitemfit}).
Since we requested a saturated and independence model (\verb\mxRefModels\; line~\ref{e3:refModels}),
CFI, TLI, and RMSEA are available in the \pkg{OpenMx} summary
and suggest adequate model fit.
The relative indices of fit are appropriate for these data
...
If your interest is Rasch models (all slope parameters constrained equal)
...
\section{Item factor analysis}\label{section:ifa}
...
print(pl)
@ 
\caption{Log $l^2$-norm of error by quadrature width and number of points
...
influences accuracy and optimization time.
\pkg{OpenMx} uses an equal interval quadrature grid for
...
\end{equation}
%
For two factor models, a varimax rotation was applied.
...
Without knowing more about how the data was collected,
parsimony favors a single factor model without constraints on the
slopes.
We can further check this idea by comparison
of two factor models with and without the slope constraint.

<<fig.height=3,cache=TRUE,echo=TRUE>>=
...
grid.arrange(plotTwoFactors(m2$itemModel$item$values[1:2,]) + labs(title="a."),
             plotTwoFactors(m4$itemModel$item$values[1:2,]) + labs(title="b."), ncol=2)
...
it can be helpful to plot item factor loadings after a varimax rotation
and then superimpose promax axes \citep[p.~265]{bock1988}.
The \verb\plotTwoFactors\ function implements this plot and
both models are plotted above.
...
many latent factors is known but not yet implemented in \pkg{OpenMx} \citep{cai2010b}.
The model builder offer as many as five factors because,
as long as the secondary factors are disjoint (not general factors),
they count as a single factor with respect to estimation time \citep{cai2010}.
