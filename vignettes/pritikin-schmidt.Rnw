...
print(pl)
@ 
\caption{How dichotomous data is converted into continuous data conditional on examinee skill.}
...
with the OpenMP shared memory multiprocessing extension enabled
on non-Microsoft Windows operating systems.
As soon as Windows supports the OpenMP then we expect performance
