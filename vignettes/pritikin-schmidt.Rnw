...
The item may be hurt by awkward wording,
...
corresponds with difficulty.
However, as item models have grown more flexible,
parameter values and intuitive psychological
interpretation has become more distant.
...
Refer to \citet{loken2010} for a detailed discussion of the psychological meaning
of $g$ and $u$.
By default, the nominal model uses \emph{trend} for
the \verb\T.a\ and \verb\T.c\ matrices.
...
The local dependence is most severe between items
V7-V12, however, it is not clear why.
...
a data set from \citet{thissen1988} available as an online supplement.
Click the \verb\Row names?\ checkbox in the control panel
to disable row names.
