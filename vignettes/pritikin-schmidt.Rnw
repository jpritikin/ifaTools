...
However, the app does not conduct any analysis but generates an analysis script.
...
%
\begin{align}
\Prob(\pick=0| a,b,g,u,\tau) &= 1- \Prob(\pick=1| a,b,g,\tau) \\ \label{eqn:dichotomous}
...
A \emph{\#PL} naming shorthand has developed to refer to versions of the dichotomous
...
all slope parameters are constrained equal and the variance is fixed to 1.0,
or equivalently,
